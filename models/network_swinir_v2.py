...
#from models.softmax_free_transformer import SoftmaxFreeTransformer, SoftmaxFreeTrasnformerBlock
...
class small_Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=16,out_features=1):
        super().__init__()
        self.fc1 = nn.Linear(in_features, hidden_features)
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        #######swin transformer V2 updation###########
        ##################start#######################
        #learnable parameter theta in scaled cosine attention-swin transformer V2
    ...
    def forward(self, x, mask=None):
        ...
        attn=torch.div(torch.cos(attn),self.theta)
        #Continous relative position bias by meta network(small MLP)- swin transformer V2
        meta_bias_table=self.meta_network(self.linear_spaced_index)
        trunc_normal_(meta_bias_table, std=.02)
        relative_position_bias = meta_bias_table[self.relative_position_index.view(-1)].view(
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, x_size):
        ...
        # FFN
        #post normalization-swin transformer V2
        x=self.norm1(x)
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
                 ...
                 drop_path=0., norm_layer=nn.LayerNorm,newton_max_iter=20, kernel_method="cuda",
        ...
        #SOFM module
        # dpr = [x.item() for x in torch.linspace(0, 0.1, sum(self.depths))]
        # self.blocks = nn.ModuleList([SoftmaxFreeTrasnformerBlock(
        #     dim=60, num_heads=10, drop_path=dpr[i], H=64, W=64, conv_size=3,
